...
def detect_image_format(image_path):
    with open(image_path, 'rb') as f:
        ...
        format_dict = {
            ...
            'WebP': b'MM*\x00', 
            'ICO': b'RIFF', 
            'PSD': b'\x00\x00\x01\x00', 
            '8BPS': b'8BPS',
            'SVG': b'<svg'
        }
        format_score = {}
        need_fix = False
        _print("current_header(hex): "+file_header.hex())
        for i in format_dict:
            header = format_dict[i]
            length = len(header)
            score = ratio(file_header[:length].hex(), header.hex())
            if score == 100:
                return i, need_fix
            elif score < 40:
                continue
            else:
                format_score[i] = score
        if not format_score:
            return False, need_fix
        else:
        ...
        #     need_fix = True
        # elif
        # if file_header.startswith(b'\xff\xd8\xff\xe0'):
        #     return "JPEG", need_fix
        ...
        #     return "TIFF", need_fix
        # elif file_header.startswith(b'RIFF'):
        #     return "WebP", need_fix
        ...
        # elif score < 40:
        #     return False, need_fix
def fix_header(image_content, correct_format):
    format_dict = {
        'JPEG': b'\xff\xd8\xff\xe0', 
        'PNG': b'\x89\x50\x4e\x47\x0d\x0a\x1a\x0a', 
        'GIF': b'\x47\x49\x46\x38\x39\x61',
    ...
    correct_header = format_dict[correct_format]
    length = len(correct_header)
    if image_content[:length] == correct_header:
        return image_content # Impossible to here XD
    else:
        correct_header_hex = correct_header.hex()
        current_header = image_content[:length+2]
        current_header_hex = current_header.hex()
        if correct_header_hex in current_header_hex:
            start_index = current_header.find(correct_header)
            correct = current_header[start_index:]
            image_content = correct+ image_content[start_index+length+1:] 
        else:
            pass #TODO
        return correct_header + image_content[length:]